...
def process_results(responses1, simulation_data):
    results =[]
    for response in responses1:
        if response in simulation_data:
            # print(str(simulation_data[response][index]))
            results.append(str(simulation_data[response]))
        else:
            print(f"Warning: '{response}' not found in simulation data.")
            results.append('0.0')  # Default to zeros if not found
    
    print("The maximum Drift along x is", results)
