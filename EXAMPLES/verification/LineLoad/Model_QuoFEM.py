...
def run_analysis():
    ...
    wind_field_params[0]  = Vel*m/sec # [m/s]      Um: reference radial velocity
    wind_field_params[1]  = Rad*m  # [m]        rm: reference radius
    wind_field_params[2]  = Height_ref*m  # [m]        zm: reference height
    wind_field_params[3]  = Swirl   #             S: swirl ratio (ratio of max circumferential velocity to radial velocity at reference height)
    ...
    for i in range(len(layer_in)):
            ...
            #  element("LineLoad", k+1+n_members, int(connect_in[j][0]), int(connect_in[j][1]), radius_of_gyration, ParticleDynamics.library_name) # [m] (LineLoad, LineLoadID, node1, node2, radius, library)
                if file_format =="exo":
                    ...
                    op.element("LineLoad", k+1+n_members, int(layer_in[i][j][0]), int(layer_in[i][j][1]), radius_of_gyration, ParticleDynamics.library_name) # [m] (LineLoad, LineLoadID, node1, node2, radius, library)
                    ...
                    op.element("LineLoad", k+1+n_members, layer_in[i][j][0], layer_in[i][j][1], radius_of_gyration, ParticleDynamics.library_name) # [m] (LineLoad, LineLoadID, node1, node2, radius, library)
                k+=1
    ...
    Drift = (time_series1[:, 1])
    Time = (time_series1[:, 0])

    
    node_disp = [[op.nodeDisp(node_i, dof_j) 
	             for dof_j in [1,2,3]] for node_i in int(push[-1][0])]
